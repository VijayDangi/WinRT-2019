...
LRESULT CALLBACK SearchRecordWndProc (HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	static HWND hwndButtonExport;
	static int  flagExport = 0;
 ...
 *******************************************************/					
		...
		//Export
				else if(LOWORD(wParam) == ID_BUTTON_EXPORT  && HIWORD(wParam) == BN_CLICKED)
				{
					...
					{
						//show list window
						ShowWindow(hwndListExport, SW_SHOW);
						flagExport = 1;				
					}
				}
				
				else if(LOWORD(wParam) == ID_LIST_EXPORT)
				{
					if(HIWORD(wParam) == LBN_SELCHANGE)
					{
						int index = AMC_SetSelectedText(hwndListExport, hwndStaticExport);
				...
				else if(LOWORD(wParam) == ID_EXPORT_BUTTON && HIWORD(wParam) == BN_CLICKED)
				{
					 if(employee == NULL)
					 {
						 MessageBox(hwnd, TEXT("There is no data to export!!"), TEXT("ERROR"), MB_ICONERROR);
						 return(0);
					 }
						 
					 int index = SendMessage(hwndListExport, LB_GETCURSEL, (WPARAM) 0, (LPARAM) 0);
					 
					 if(index == 0) // *.csv
					 {
						 EnableWindow(hwndExportButton, FALSE);
						 
						 int status = AMC_ExportInCSVFileFormate(hwnd, employee);
						 if(status == 0)
						 {
