/*
 * Writing DefinitionModuleExplicit.exe using Module Definition File method by explicit linking.
 */
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE prevInstance, LPSTR iCmdLine, int iCmdShow)
{
	...
	HWND hwnd;
	MSG msg;
	TCHAR szAppName[] = TEXT("MyMathUsingDefExplicit");
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT iMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_CREATE:
		...
		//load dll
		hDll = LoadLibrary(TEXT("MyMathUsingDllDef.dll"));

		if (hDll == NULL)
		{
			MessageBox(hwnd, TEXT("MyMathUsingDllDef.dll Not Found"), TEXT("ERROR"), MB_OK);
			DestroyWindow(hwnd);
		}

		//find "MakeSquare() function in dll and returns it's base address
